...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        requested_JSON = requests.get(f"{ENDPOINT}action=parse&section=1&page=jodi&format=json").json()
        ...
        returnString = JodiHandler.parse_currentWeekday(text, weekday.lower())
        return returnString
    ...
    def parse_currentWeekday(text, weekday):
        ...
        for idx, word in enumerate(splitText):
            if (word in WEEKDAYS)or (word[0: len(word) -1] in WEEKDAYS):
                daySet = JodiHandler.parse_dayset(splitText, idx)

            if (weekday in daySet): 
                startIdx = idx
                break

        return JodiHandler.build_return_schedule(splitText, startIdx)
