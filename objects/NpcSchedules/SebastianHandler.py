...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        requested_JSON = requests.get(f"{ENDPOINT}action=parse&section=1&page=sebastian&format=json").json()
        ...
        returnString = SebastianHandler.parse_currentWeekday(text, weekday.lower())
        return returnString
    ...
    def parse_currentWeekday(text, weekday):
        ...
        for idx, word in enumerate(splitText):
            ...
            if (word in WEEKDAYS)or (word[0: len(word) -1] in WEEKDAYS):
                daySet = SebastianHandler.parse_dayset(splitText, idx)
        ...
        return SebastianHandler.build_return_schedule(splitText, startIdx)
    ...
    def build_return_schedule(splitText, startIdx):
        ...
        for word in splitText[startIdx:]:
            ...
            if (not switchedFromDays):
                if (word[0:len(word)-1] in WEEKDAYS) or (word in WEEKDAYS) and first or word == "Regular":
                    returnString += word
                    first = False
                elif (word[0:len(word)-1] in WEEKDAYS) or (word in WEEKDAYS) or (word == 'and') or word == "Schedule":
                    returnString += ' ' + word
                else: 
                ...
                if (word == 'Time' or word == 'Location'):
                    continue
                elif (word in WEEKDAYS) or (word[0:len(word) -1] in WEEKDAYS) or (word == "Rainy") or (word == "Regular") or (word == 'Birthday'):
                    break
                else:
