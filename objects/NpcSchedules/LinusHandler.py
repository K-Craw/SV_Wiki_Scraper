...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        requested_JSON = requests.get(f"{ENDPOINT}action=parse&section=1&page=linus&format=json").json()
        ...
        returnString = LinusHandler.parse_currentWeekday(text, weekday.lower())
        return returnString
    ...
    def parse_currentWeekday(text, weekday):
        ...
        for idx, word in enumerate(splitText):
            ...
            if (word in WEEKDAYS)or (word[0: len(word) -1] in WEEKDAYS):
                daySet = LinusHandler.parse_dayset(splitText, idx)
        ...
        return LinusHandler.build_return_schedule(splitText, startIdx)
