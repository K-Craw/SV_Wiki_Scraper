...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        requested_JSON = requests.get(f"{ENDPOINT}action=parse&section=1&page=Kent&format=json").json()
        ...
        if (not found):
            for data in df:
                ...
                if ApiHandler.contains(keys, 'Regular Schedule'):
                    ...
                    returnString = KentHandler.build_return_schedule(timeTxt, locationTxt, 'Regular Schedule')
