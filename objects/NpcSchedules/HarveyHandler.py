...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        requested_JSON = requests.get(f"{ENDPOINT}action=parse&section=1&page=harvey&format=json").json()
        ...
        returnString = HarveyHandler.parse_currentWeekday(text, weekday.lower())
        return returnString
    ...
    def parse_currentWeekday(text, weekday):
        ...
        return HarveyHandler.build_return_schedule(splitText, startIdx)
