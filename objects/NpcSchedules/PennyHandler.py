...
#----------------------------------SCHEDULE METHODS----------------------------------------------------------------
    ...
    async def get_schedule(season, weekday):
        ...
        returnString = PennyHandler.parse_currentWeekday(text, weekday.lower())
        return returnString
    ...
    def parse_currentWeekday(text, weekday):
        ...
        for idx, word in enumerate(splitText):
            if (word in WEEKDAYS)or (word[0: len(word) -1] in WEEKDAYS):
                daySet = PennyHandler.parse_dayset(splitText, idx)

            if (weekday in daySet): 
                startIdx = idx
                break

        return PennyHandler.build_return_schedule(splitText, startIdx)
